...
public class BookingRestController {
    ...
    /** Current user's own bookings only, newest first. */
    @GetMapping("/my")
    @PreAuthorize("hasAuthority('bookings:view')")
    public List<BookingDto> myBookings(@AuthenticationPrincipal AuthUserDetails principal) {
        return bookingRepository.findByBookedByUserIdOrderByCreatedAtDesc(principal.getUserId()).stream()
                .map(b -> enrich(bookingMapper.toDto(b), b.getBookedByUserId()))
                .collect(Collectors.toList());
    }

    /**
     * Approval queue — PENDING bookings from direct children only, newest first.
    ...
    public List<BookingDto> queue(@AuthenticationPrincipal AuthUserDetails principal) {
        ...
        List<Booking> bookings = bookingRepository.findByStatusAndBookedByUserIdInOrderByCreatedAtDesc(
    ...
    public BookingDto create(@RequestBody CreateBookingRequest request,
        ...
        // Agency admins (level 3) can confirm their own bookings — no approval from parent needed
        if (principal.getUserTypeLevel() == 3) {
            saved = bookingService.confirm(saved.getId(), null, principal.getUserId());
        }
    ...
    private PricingInput buildPricingInput(CreateBookingRequest request, List<Passenger> passengers, Long userId) {
        ...
        switch (request.bookableType().toLowerCase()) {
            case "flight" -> {
                ...
                fareAdult  = nvl(f.getFareAdult());
                fareChild  = nvl(f.getFareChild());
                fareInfant = nvl(f.getFareInfant());
                if (total > 0 && f.getTaxTotal() != null) {
                    taxPerPassenger = f.getTaxTotal()
            ...
            case "custom" -> {
                CustomPackage cp = customPackageRepository.findById(request.bookableId())
                        .orElseThrow(() -> new EntityNotFoundException("Custom package not found: " + request.bookableId()));
                if (cp.getQuotaTotal() != null) {
                    int occupied = bookingRepository
                            .findByBookableTypeAndBookableId("custom", cp.getId())
                            .stream()
                            .filter(b -> b.getStatus() != BookingStatus.CANCELLED)
                            .mapToInt(b -> passengerRepository.findByBookingId(b.getId()).size())
                            .sum();
                    int available = Math.max(0, cp.getQuotaTotal() - occupied);
                    if (available < total) {
                        throw new IllegalStateException(
                                "Not enough quota. Only " + available + " spot(s) remaining.");
                    }
                }
                fareAdult  = nvl(cp.getBasePrice());
                fareChild  = cp.getPriceChild()  != null ? cp.getPriceChild()  : nvl(cp.getBasePrice());
                fareInfant = cp.getPriceInfant() != null ? cp.getPriceInfant() : nvl(cp.getBasePrice());
            }
        }
    ...
    public ResponseEntity<byte[]> downloadInvoice(@PathVariable Long id) {
        Booking booking = bookingService.findById(id);
        invoiceService.generateInvoicePdf(id, booking);
    ...
    private LedgerEntryDto toLedgerDto(LedgerEntry e) {
        ...
        java.math.BigDecimal bookingGrossTotal = null;
        java.math.BigDecimal bookingNetTotal = null;
        String bookableType = null;
        if (e.getBookingId() != null) {
            var bookingOpt = bookingRepository.findById(e.getBookingId());
            if (bookingOpt.isPresent()) {
                Booking b = bookingOpt.get();
                bookableTitle = resolveBookableTitle(b.getBookableType(), b.getBookableId());
                bookedByUserId = b.getBookedByUserId();
                bookedByName = b.getBookedByUserId() == null ? null : userRepository.findById(b.getBookedByUserId())
                        .map(u -> u.getFirstName() + " " + u.getLastName()).orElse(null);
                approvedByUserId = b.getApprovedByUserId();
                approvedByName = b.getApprovedByUserId() == null ? null : userRepository.findById(b.getApprovedByUserId())
                        .map(u -> u.getFirstName() + " " + u.getLastName()).orElse(null);
                bookingStatus = b.getStatus().name();
                bookingGrossTotal = b.getGrossTotal();
                bookingNetTotal = b.getNetTotal();
                bookableType = b.getBookableType();
            }
        }
        return new LedgerEntryDto(
                ...
                e.getCreatedAt(),
                bookableTitle,
                bookedByUserId, bookedByName,
                approvedByUserId, approvedByName,
                bookingStatus, bookingGrossTotal, bookingNetTotal, bookableType
        );
    }
}
