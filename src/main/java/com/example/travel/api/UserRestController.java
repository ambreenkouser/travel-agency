...
public class UserRestController {
    ...
    /**
     * Own-created list: every user sees only the users they directly created (parentId = myId).
     *  level 1 super_admin  → direct children only
     *  level 2 master_agent → direct children only
     *  level 3 agency_admin → direct children only
     *  level 4 sub_agent    → only themselves
     */
    @GetMapping
    ...
    public List<UserListDto> list(@AuthenticationPrincipal AuthUserDetails principal) {
        ...
        List<User> users = myLevel >= 4
                ? userRepository.findById(myId).map(List::of).orElse(List.of())
                : userRepository.findByParentId(myId);
