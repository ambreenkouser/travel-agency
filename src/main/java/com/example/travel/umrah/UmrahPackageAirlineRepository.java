...
public interface UmrahPackageAirlineRepository extends JpaRepository<UmrahPackageAirline, Long> {
    ...
    /** Sum of allocatedSeats across all packages for a given airline (0 if none). */
    @Query("SELECT COALESCE(SUM(upa.allocatedSeats), 0) FROM UmrahPackageAirline upa WHERE upa.airline.id = :airlineId")
    int sumAllocatedSeatsByAirlineId(@Param("airlineId") Long airlineId);
}
