...
const empty = { code: '', name: '', seatQuota: '' }
...
export default function AirlinesPage() {
  ...
  function openEdit(a) {
    setForm({ code: a.code ?? '', name: a.name ?? '', seatQuota: a.seatQuota ?? '' })
    setEditing(a.id); setShowForm(true); setError('')
  }

  async function handleSubmit(e) {
    e.preventDefault(); setError('')
    const payload = { ...form, seatQuota: form.seatQuota === '' ? null : Number(form.seatQuota) }
    try {
      editing ? await updateAirline(editing, payload) : await createAirline(payload)
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-gray-900">Airlines</h1>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-6">
            <h2 className="text-lg font-semibold mb-4">{editing ? 'Edit Airline' : 'New Airline'}</h2>
            <form onSubmit={handleSubmit} className="space-y-3">
              <label className="block text-sm">
                <span className="block text-gray-600 mb-1">IATA Code * (2–3 chars)</span>
                <input
                  value={form.code}
              ...
              <label className="block text-sm">
                <span className="block text-gray-600 mb-1">Seat Quota (Umrah allocation)</span>
                <input
                  type="number" min="0" value={form.seatQuota}
                  onChange={e => setForm(f => ({ ...f, seatQuota: e.target.value }))}
                  placeholder="e.g. 200"
                  className="w-full border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
              </label>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {airlines.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400">No airlines found</td></tr>
              )}
              {airlines.map(a => (
                <tr key={a.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono font-medium text-gray-900">{a.code}</td>
                  <td className="px-4 py-3 text-gray-700">{a.name}</td>
                  <td className="px-4 py-3 text-gray-500">{a.seatQuota ?? '—'}</td>
