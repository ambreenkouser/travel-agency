      ...
      {modal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 overflow-y-auto max-h-[90vh]">
            ...
            <p className="text-sm text-gray-500 mb-4">Booking #{modal.booking.id}</p>
              ...
              {modal.booking.passengers?.length > 0 && (
                <div>
                  <span className="text-gray-400 text-xs">
                    {modal.booking.passengers.map(p => `${p.firstName} ${p.lastName} (${p.type})`).join(', ')}
                  </span>
                </div>
              )}
                ...
                {modal.booking.payment.hasSlip && (
                  <div>
                    ...
                    <img
                      ...
                      alt="Payment slip"
                      className="max-h-48 rounded border border-gray-200 object-contain w-full"
                    />
                  </div>
                )}
              </div>
