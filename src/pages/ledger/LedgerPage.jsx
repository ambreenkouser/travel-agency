...
export default function LedgerPage() {
  ...
  const filtered = entries.filter(e => {
    ...
    return (
      ...
      (e.bookableTitle ?? '').toLowerCase().includes(q) ||
    ...
    <div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['#', 'Booking / Item', 'Booked By', 'Type', 'Amount', 'Date'].map(h => (
              ...
              {filtered.map(e => {
                ...
                return (
                  <tr
                    ...
                    className={`hover:bg-blue-50/40 cursor-pointer ${isAdj ? 'bg-blue-50/30' : ''}`}
                  >
                    ...
                    <td className="px-4 py-3">
                      {e.bookingId
                        ? (
                          <div>
                            <span className="font-mono text-blue-600 font-medium text-xs">#{e.bookingId}</span>
                            {e.bookableTitle && (
                              <div className="text-xs text-gray-500 truncate max-w-[180px]">{e.bookableTitle}</div>
                            )}
                          </div>
                        )
                    ...
                    <td className="px-4 py-3 text-gray-600 text-xs">
                      {e.bookedByName ?? e.userName ?? '—'}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${
                        e.entryType === 'DEBIT' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                      }`}>
                        {e.entryType}
                      </span>
                    </td>
      ...
      {detailEntry && (
        <div className="fixed inset-0 z-40">
          <div className="absolute inset-0 bg-black/20" onClick={() => setDetailEntry(null)} />
          <div className="absolute top-0 right-0 h-full w-96 max-w-full bg-white shadow-2xl overflow-y-auto z-50 flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 sticky top-0 bg-white">
              <h2 className="text-base font-semibold text-gray-900">
                {detailEntry.bookingId ? `Booking #${detailEntry.bookingId}` : 'Adjustment'} — Entry #{detailEntry.id}
              </h2>
              <button onClick={() => setDetailEntry(null)} className="text-gray-400 hover:text-gray-600 text-2xl font-bold leading-none">×</button>
            </div>

            <div className="px-6 py-5 space-y-5 flex-1">
              {/* Entry details */}
              <section>
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Ledger Entry</p>
                <dl className="space-y-2 text-sm">
                  <DRow label="Type">
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${
                      detailEntry.entryType === 'DEBIT' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                    }`}>{detailEntry.entryType}</span>
                  </DRow>
                  <DRow label="Amount">
                    <span className={`font-bold ${detailEntry.entryType === 'DEBIT' ? 'text-red-600' : 'text-green-600'}`}>
                      {detailEntry.entryType === 'DEBIT' ? '−' : '+'} {detailEntry.currency} {Number(detailEntry.amount).toLocaleString()}
                    </span>
                  </DRow>
                  {detailEntry.memo && <DRow label="Memo" value={detailEntry.memo} />}
                  <DRow label="Date" value={new Date(detailEntry.createdAt).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' })} />
                </dl>
              </section>

              {/* Booking details */}
              {detailEntry.bookingId && (
                <section className="border-t border-gray-100 pt-4">
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Booking Details</p>
                  <dl className="space-y-2 text-sm">
                    {detailEntry.bookableTitle && (
                      <DRow label="Item" value={detailEntry.bookableTitle} valueClass="text-blue-700 font-medium" />
                    )}
                    {detailEntry.bookableType && (
                      <DRow label="Type" value={detailEntry.bookableType.charAt(0).toUpperCase() + detailEntry.bookableType.slice(1)} />
                    )}
                    {detailEntry.bookingStatus && (
                      <DRow label="Status">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${
                          detailEntry.bookingStatus === 'CONFIRMED' ? 'bg-green-100 text-green-700' :
                          detailEntry.bookingStatus === 'CANCELLED' ? 'bg-red-100 text-red-700' :
                          'bg-yellow-100 text-yellow-700'
                        }`}>{detailEntry.bookingStatus}</span>
                      </DRow>
                    )}
                    {detailEntry.bookingGrossTotal != null && (
                      <DRow label="Gross Total" value={`PKR ${Number(detailEntry.bookingGrossTotal).toLocaleString()}`} />
                    )}
                    {detailEntry.bookingNetTotal != null && (
                      <DRow label="Net Total" value={`PKR ${Number(detailEntry.bookingNetTotal).toLocaleString()}`} />
                    )}
                  </dl>
                </section>
              )}
              ...
              {/* Persons */}
              {(detailEntry.bookedByName || detailEntry.approvedByName) && (
                <section className="border-t border-gray-100 pt-4">
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">People</p>
                  <dl className="space-y-2 text-sm">
                    {detailEntry.bookedByName && (
                      <DRow label="Booked By" value={detailEntry.bookedByName} valueClass="font-medium text-gray-800" />
                    )}
                    {detailEntry.approvedByName && (
                      <DRow label="Approved By" value={detailEntry.approvedByName} valueClass="font-medium text-gray-800" />
                    )}
                  </dl>
                </section>
              )}
            ...
            <div className="px-6 py-4 border-t border-gray-200">
              <button onClick={() => setDetailEntry(null)} className="w-full py-2 border text-sm rounded-md hover:bg-gray-50">Close</button>
            </div>
          </div>
        </div>
      )}
...
function DRow({ label, value, valueClass, children }) {
  return (
    <div className="flex justify-between gap-4">
      <dt className="text-gray-500 shrink-0">{label}</dt>
      <dd className={`text-right ${valueClass ?? 'text-gray-800'}`}>{children ?? value ?? '—'}</dd>
    </div>
  )
}
